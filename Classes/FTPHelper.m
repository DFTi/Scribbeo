    ...
    switch (type) {
        ...
        case kCFStreamEventErrorOccurred:
            error = CFWriteStreamCopyError(info->writeStream);
 			DELEGATE_CALLBACK(dataUploadFailed:, (id) CFErrorCopyDescription (error));
            CFRelease (error);
            goto exit;
        case kCFStreamEventEndEncountered:
            fprintf(stderr, "\nUpload complete\n");
...
static Boolean MySimpleUpload(CFStringRef uploadDirectory, CFURLRef fileURL, CFStringRef username, CFStringRef password, BOOL audioFlag)
{
    CFWriteStreamRef       writeStream;
    CFReadStreamRef        readStream;
    CFStreamClientContext  context = { 0, NULL, NULL, NULL, NULL };
    CFURLRef               uploadURL, destinationURL;
    CFStringRef            fileName, fileName0;
    Boolean                success = true;
    MyStreamInfo           *streamInfo;
    ...
    if ([ (VideoTreeAppDelegate *) [[UIApplication sharedApplication] delegate] BonjourMode] )
        home = @"/VideoTree";
    if (! audioFlag) 
        fileName = (CFStringRef) [[NSString stringWithFormat: @"%@/Notes/%@", home, fileName0] stringByReplacingOccurrencesOfString: @"%20" withString: @" "];
    else
        fileName = (CFStringRef) [[NSString stringWithFormat: @"%@/Notes/audio/%@", home, fileName0] stringByReplacingOccurrencesOfString: @"%20" withString: @" "];
        
    CFRelease (fileName0);
    assert(fileName != NULL);
    
    NSLog (@"MySimpleUpload: Uploading to %@", fileName);
 ...
 // CFRelease(fileName);
    ...
    success = CFReadStreamOpen(readStream);
    if (success) {
        NSLog2 (@"CFReadStreamOpen succeeded");
        ...
        if (success) {
            NSLog2 (@"CFWriteStreamSetClient succeeded");
            ...
            if (success == false) {
                ...
                NSLog2 (@"CFWriteStreamOpen opened non-blocking");
        } else {
...
{	
	...
	BOOL audioFlag = [[fileLocation pathExtension] isEqualToString: @"aac"] || [[fileLocation pathExtension] isEqualToString: @"mov"];
	...
	MySimpleUpload((CFStringRef)self.urlString, readURL, (CFStringRef)uname, (CFStringRef)pword, audioFlag);
	CFRelease(readURL);
}
...
+ (void) upload: (NSString *) anItem
{
    ...
    [[appDel viewController] uploadActivityIndicator: YES];
...
static void MyDownloadCallBack(CFReadStreamRef readStream, CFStreamEventType type, void * clientCallBackInfo)
{
 ...
 //             fprintf(stderr, "File size is unknown. Uh oh.\n");
        ...
        case kCFStreamEventErrorOccurred:
            ...
            fprintf(stderr, "CFReadStreamGetError returned (%d, %ld)\n", (int) error.domain, (long int) error.error);
...
//          fprintf(stderr, "\nDownload complete\n");
            goto exit;
        default:
            fprintf(stderr, "Received unexpected CFStream event (%d)", (int) type);
            break;
    }
    return;
...
static Boolean MySimpleDownload(CFStringRef urlString, CFURLRef downloadPath, CFStringRef username, CFStringRef password)
{
    CFReadStreamRef        readStream;
    CFWriteStreamRef       writeStream;
    CFStreamClientContext  context = { 0, NULL, NULL, NULL, NULL };
...
- (void) fetch: (NSString *) anItem to: (NSString *) writepath
{
	if (!self.uname || !self.pword) COMPLAIN_AND_BAIL(@"Please set user name and password first");
	if (!self.urlString) COMPLAIN_AND_BAIL(@"Please set URL string first");
...
+ (void) download: (NSString *) anItem to: (NSString *) path
{
	[sharedInstance fetch:[anItem stringByReplacingOccurrencesOfString: @"%20" withString: @" "] to: path];
}
